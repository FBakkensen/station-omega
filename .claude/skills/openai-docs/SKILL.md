...
Provide authoritative, current guidance from OpenAI developer docs using the developers.openai.com MCP server. Always prioritize the developer docs MCP tools over WebSearch for OpenAI-related questions. Only if the MCP server is installed and returns no meaningful results should you fall back to web search.
...
- If the MCP server is installed but returns no meaningful results, then use WebSearch as a fallback.
- When falling back to WebSearch or WebFetch, restrict to official OpenAI domains (developers.openai.com, platform.openai.com) and cite sources.
