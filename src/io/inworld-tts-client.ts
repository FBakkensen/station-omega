...
export class InworldTTSClient implements TTSClient {
  ...
  async generateSpeech(request: TTSRequest): Promise<TTSResult> {
    const response = await this.fetchFn(`${INWORLD_API_BASE}/voice:stream`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return { ok: false, error: `Inworld API error: ${errorText}`, status: 502 };
    }

    const reader = response.body?.getReader();
    if (!reader) {
      return { ok: false, error: 'No response body', status: 502 };
    }
    ...
    if (textBuffer.trim()) {
      try {
        const parsed = JSON.parse(textBuffer) as { result?: { audioContent?: string } };
        if (parsed.result?.audioContent) {
          const audioChunk = Buffer.from(parsed.result.audioContent, 'base64');
    ...
    return { ok: true, wavBase64: wavBuffer.toString('base64') };
  }
}
